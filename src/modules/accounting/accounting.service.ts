...
export class AccountingService implements OnModuleInit {
  ...
  private async migrateOrphanedRecords(company: Company) {
    console.log(
      '[AccountingService] Migrating orphaned records to default company...',
    );
  ...
  private async ensureDefaultPeriod() {
    ...
    const period = await this.periodRepository.findOne({
      where: { year, month, company: { id: company.id } },
    });
    if (!period) {
  ...
  async validatePeriod(date: Date, companyId: string) {
    ...
    if (period && period.status === PeriodStatus.CLOSED) {
      throw new Error(
        `El periodo contable ${year}-${month} está CERRADO. No se permiten movimientos.`,
      );
    }
  }
  ...
  async getCompany() {
    ...
    if (!company)
      throw new Error('Company not found. Check x-company-id header.');
    return company;
  }
  ...
  async getTrialBalance(startDate: string, endDate: string) {
    ...
    for (const account of accounts) {
      const legacyBefore = await this.transactionRepository
        .createQueryBuilder('tx')
        .select('SUM(tx.amount)', 'total')
        .where(
          '(tx.debitAccountId = :id OR tx.creditAccountId = :id) AND tx.date < :start',
          { id: account.id, start },
        )
        .getRawOne();

      const entriesBefore = await this.journalEntryRepository
        .createQueryBuilder('je')
        .leftJoin('je.voucher', 'v')
        .select('SUM(je.debit - je.credit)', 'total')
        .where('je.accountId = :id AND v.date < :start', {
          id: account.id,
          start,
        })
        .getRawOne();
      ...
      const initialBalance =
        Number(legacyBefore?.total || 0) + Number(entriesBefore?.total || 0);

      const legacyDebitPeriod = await this.transactionRepository.sum('amount', {
        debitAccount: { id: account.id } as any,
        date: Between(start, end),
      });
      const legacyCreditPeriod = await this.transactionRepository.sum(
        'amount',
        {
      ...
      const entriesPeriodDetail = await this.journalEntryRepository
        .createQueryBuilder('je')
        .leftJoin('je.voucher', 'v')
        .select('SUM(je.debit)', 'debit')
        .addSelect('SUM(je.credit)', 'credit')
        .where('je.accountId = :id AND v.date BETWEEN :start AND :end', {
          id: account.id,
          start,
          end,
        })
      ...
      const periodDebit =
        Number(legacyDebitPeriod || 0) +
        Number(entriesPeriodDetail?.debit || 0);
      const periodCredit =
        Number(legacyCreditPeriod || 0) +
        Number(entriesPeriodDetail?.credit || 0);
  ...
  async createVoucher(data: any, userId: string) {
    ...
    try {
      ...
      if (Math.abs(totalDebit - totalCredit) > 0.01) {
        throw new Error(
          'El comprobante no está balanceado. Débitos != Créditos',
        );
      }
      ...
      for (const item of entries) {
        ...
        if (!account)
          throw new Error(`Cuenta no encontrada: ${item.accountId}`);

        // Validaciones según configuración de la cuenta (PUC)
        if (account.requiresThirdParty && !item.thirdPartyId) {
          throw new Error(
            `La cuenta ${account.code} - ${account.name} requiere obligatoriamente un Tercero.`,
          );
        }
        if (account.requiresCostCenter && !item.costCenterId) {
          throw new Error(
            `La cuenta ${account.code} - ${account.name} requiere obligatoriamente un Centro de Costo.`,
          );
        }
        ...
        account.balance =
          Number(account.balance) + (Number(item.debit) - Number(item.credit));
  ...
  async getBudgetVsExecution(year: number) {
    ...
    return budgets.map((b) => {
      const exec = execution.find(
        (e) =>
          e.accountId === b.account.id && e.costCenterId === b.costCenter?.id,
      );
  ...
  async reconcileBank(id: string, journalEntryIds: string[]) {
    ...
    for (const entryId of journalEntryIds) {
      await this.journalEntryRepository.update(entryId, {
  ...
  async getReconciliationReport(
    ...
    return {
      ...
      reconciledBalance: reconciled.reduce(
        (sum, e) => sum + Number(e.debit || 0) - Number(e.credit || 0),
        0,
      ),
      pendingBalance: pending.reduce(
        (sum, e) => sum + Number(e.debit || 0) - Number(e.credit || 0),
        0,
      ),
    };
  ...
  async getRecurringEntries(status?: RecurrenceStatus) {
    ...
    return this.recurringEntryRepository.find({
      where,
  ...
  async processRecurringEntries() {
    ...
    return {
      ...
      message: 'Recurring entries processed',
    };
  }
  ...
  async getProjectFinancialSummary(projectId: string) {
    ...
    const transactions = await this.projectTransRepository.find({
      where: { projectId },
    });
