...
export class PosService {
  ...
  async getDailySummary(date?: string) {
    ...
    const todaySales = sales.filter(
      (s) => s.createdAt >= targetDate && s.createdAt < endDate,
    );

    const totalCash = todaySales
      .filter((s) => s.paymentMethod === PaymentMethodPOS.CASH)
      .reduce((sum, s) => sum + Number(s.total), 0);
    const totalCard = todaySales
      .filter((s) => s.paymentMethod === PaymentMethodPOS.CARD)
      .reduce((sum, s) => sum + Number(s.total), 0);
    const totalTransfer = todaySales
      .filter((s) => s.paymentMethod === PaymentMethodPOS.TRANSFER)
      .reduce((sum, s) => sum + Number(s.total), 0);
