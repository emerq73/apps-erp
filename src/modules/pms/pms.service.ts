...
import {
  ...
  CleaningStatus,
  CleaningType,
} from './entities/cleaning-request.entity';
import {
  HousekeepingStaff,
  StaffRole,
} from './entities/housekeeping-staff.entity';
import {
  MaintenanceRequest,
  MaintenanceStatus,
  MaintenancePriority,
...
} from './entities/maintenance-staff.entity';
import {
  RestaurantTable,
  TableStatus,
...
import {
  ...
  OrderType,
  PaymentMethod,
} from './entities/restaurant-order.entity';
import {
  LoyaltyTier,
  LoyaltyPoints,
  LoyaltyTransaction,
...
export class PmsService {
  constructor(
    ...
    @InjectRepository(CleaningRequest)
    private cleaningRepo: Repository<CleaningRequest>,
    @InjectRepository(HousekeepingStaff)
    private staffRepo: Repository<HousekeepingStaff>,
    @InjectRepository(MaintenanceRequest)
    private maintenanceRepo: Repository<MaintenanceRequest>,
    @InjectRepository(MaintenanceStaff)
    private maintenanceStaffRepo: Repository<MaintenanceStaff>,
    ...
    private orderRepo: Repository<RestaurantOrder>,
    @InjectRepository(LoyaltyTier)
    private loyaltyTierRepo: Repository<LoyaltyTier>,
    @InjectRepository(LoyaltyPoints)
    private loyaltyPointsRepo: Repository<LoyaltyPoints>,
    @InjectRepository(LoyaltyTransaction)
    private loyaltyTransRepo: Repository<LoyaltyTransaction>,
    @InjectRepository(LoyaltyRedemption)
    private loyaltyRedempRepo: Repository<LoyaltyRedemption>,
    @InjectRepository(CashDrawer) private drawerRepo: Repository<CashDrawer>,
    @InjectRepository(CashDrawerTransaction)
    private drawerTransRepo: Repository<CashDrawerTransaction>,
    @InjectRepository(ReservationAlert)
    private alertRepo: Repository<ReservationAlert>,
    @InjectRepository(PaymentSplit) private splitRepo: Repository<PaymentSplit>,
    @InjectRepository(DepositTransfer)
    private depositTransRepo: Repository<DepositTransfer>,
    @InjectRepository(SharedInventoryLink)
    private sharedInvRepo: Repository<SharedInventoryLink>,
  ...
  async updateRoomStatus(
    id: string,
    status: RoomStatus,
    data?: { issue?: string; notes?: string },
  ) {
    ...
    if (companyId) {
      if (status === RoomStatus.CLEANING) {
        await this.createCleaningRequest(companyId, {
          roomId: id,
          type: 'TURNOVER',
        });
      } else if (status === RoomStatus.MAINTENANCE) {
        await this.createMaintenanceRequest(companyId, {
          roomId: id,
          notes: 'Creado automáticamente desde el tablero',
        });
      }
    }
  ...
  async calculateRate(
    ...
    for (const rate of rates) {
      ...
      if (checkInDate >= start && checkInDate <= end) {
        if (
          nights >= rate.minNights &&
          (rate.maxNights === 0 || nights <= rate.maxNights)
        ) {
    ...
    if (appliedRate && Number(appliedRate.extraAdultPrice) > 0 && adults > 2) {
      totalRate =
        basePrice * nights +
        Number(appliedRate.extraAdultPrice) * (adults - 2) * nights;
    } else {
    ...
    return {
      ...
      breakdown: {
        ...
        extraAdults:
          appliedRate && Number(appliedRate.extraAdultPrice) > 0
            ? Number(appliedRate.extraAdultPrice) * (adults - 2) * nights
            : 0,
        children:
          appliedRate && Number(appliedRate.childPrice) > 0
            ? Number(appliedRate.childPrice) * children
            : 0,
      },
    };
  }
  ...
  async calculateCancellationPenalty(
    ...
    if (Number(applicablePolicy.penaltyPercentage) > 0) {
      penalty =
        ...
        (Number(applicablePolicy.penaltyPercentage) / 100);
    } else if (Number(applicablePolicy.penaltyNights) > 0) {
      penalty =
        Number(reservation.ratePerNight) *
        Number(applicablePolicy.penaltyNights);
  ...
  async getOccupancyDashboard(companyId: string) {
    const [available, occupied, cleaning, maintenance] = await Promise.all([
      this.roomRepo.count({
        where: { company: { id: companyId }, status: RoomStatus.AVAILABLE },
      }),
      this.roomRepo.count({
        where: { company: { id: companyId }, status: RoomStatus.OCCUPIED },
      }),
      this.roomRepo.count({
        where: { company: { id: companyId }, status: RoomStatus.CLEANING },
      }),
      this.roomRepo.count({
        where: { company: { id: companyId }, status: RoomStatus.MAINTENANCE },
      }),
    ]);
    ...
    const recentReservations = await this.reservationRepo.createQueryBuilder('r')
      ...
      .andWhere('r.status NOT IN (:...excluded)', { excluded: ['CANCELLED', 'NO_SHOW'] })
      .andWhere('r.checkOut >= :sevenDaysAgo', { sevenDaysAgo })
      .getMany();
    ...
    for (const res of recentReservations) {
      ...
      const coDate = new Date(res.checkOut).toISOString().split('T')[0];
      const dayIndex = last7Days.findIndex(d => d.date === coDate);
      if (dayIndex !== -1) {
        last7Days[dayIndex].revenue += amount;
        last7Days[dayIndex].occupied += 1;
      }
    ...
    const revpar =
      rooms.length > 0 ? Math.round(totalRevenue / rooms.length) : 0;
    ...
    for (const res of activeReservations) {
      if (res.hasRedAlert) {
        alerts.push({
          type: 'RED',
          message: res.alertMessage || 'Alerta roja',
      ...
      if (res.hasYellowAlert) {
        alerts.push({
          type: 'YELLOW',
          message: res.alertMessage || 'Alerta amarilla',
          reservationNumber: res.reservationNumber,
          roomNumber: res.room?.number,
        });
      }
    ...
    for (const res of activeReservations) {
      if (
        res.checkOut &&
        new Date(res.checkOut) < checkOutTime &&
        now > checkOutTime
      ) {
        alerts.push({
          type: 'LATE_CHECKOUT',
          message: 'Check-out vencido',
          reservationNumber: res.reservationNumber,
          roomNumber: res.room?.number,
        });
      }
    ...
    for (const reservation of activeReservations) {
      if (reservation.room && reservation.room.status !== RoomStatus.OCCUPIED) {
        await this.roomRepo.update(reservation.room.id, {
    ...
    return {
      ...
      stats: {
        ...
        avgStay:
          totalNights > 0
            ? Math.round(totalNights / (recentReservations.length || 1))
            : 0,
  ...
  async createReservation(companyId: string, data: any) {
    ...
    if (conflicts.length > 0) {
      ...
      throw new Error(
        `La habitación no está disponible. Conflicto con reserva ${conflict.reservationNumber} (${new Date(conflict.checkIn).toLocaleDateString()} - ${new Date(conflict.checkOut).toLocaleDateString()})`,
      );
    }
  ...
  async checkIn(
    ...
    if (
      ...
      throw new Error(
        'La reserva debe estar pendiente o confirmada para hacer check-in',
      );
    }
    ...
    if (today < checkInDate) {
      ...
      if (daysAhead > 1) {
        throw new Error(
          `El check-in solo se puede realizar a partir del ${new Date(reservation.checkIn).toLocaleDateString()}`,
        );
      }
    }
    ...
    if (data?.idDocumentNumber)
      updateData.idDocumentNumber = data.idDocumentNumber;
  ...
  async checkOut(
    ...
    if (isLateCheckout && !data?.forceLateCheckout && !data?.applyLateFee) {
      return {
        ...
        message:
          'El check-out es después de la fecha programada. ¿Desea aplicar cargo por late check-out?',
  ...
  async moveReservation(
    ...
    if (data.roomId || (data.checkIn && data.checkOut)) {
      ...
      if (conflicts.length > 0) {
        const conflict = conflicts[0];
        throw new Error(
          `La habitación no está disponible. Conflicto con reserva ${conflict.reservationNumber}`,
        );
      }
    }
    ...
    return {
      ...
      occupancyRate:
        roomNightsAvailable > 0
          ? Math.round((occupiedNights / roomNightsAvailable) * 100)
          : 0,
      totalRevenue,
      cancelledRevenue,
  ...
  async getRevenueReport(companyId: string, start?: string, end?: string) {
    ...
    for (const r of reservations) {
      ...
      if (ci >= startDate && co <= endDate) {
        ...
        revenueBySource[r.source] =
          (revenueBySource[r.source] || 0) + Number(r.totalAmount);
      }
    }
  ...
  async getCleaningRequests(companyId: string, status?: string, date?: string) {
    ...
    return qb
      .orderBy('req.isUrgent', 'DESC')
      .addOrderBy('req.scheduledDate', 'ASC')
      .getMany();
  }
  ...
  async getCleaningStats(companyId: string) {
    ...
    const completed = await this.cleaningRepo
      .createQueryBuilder('req')
      .where('req.companyId = :companyId', { companyId })
    ...
    const verified = await this.cleaningRepo
      .createQueryBuilder('req')
      .where('req.companyId = :companyId', { companyId })
  ...
  async updateCleaningRequest(id: string, data: any) {
    ...
    if (data.status === CleaningStatus.IN_PROGRESS && !req.startedAt) {
      ...
      if (!req.scheduledTime)
        req.scheduledTime = new Date().toTimeString().slice(0, 5);
    }
          ...
          if (reservation) {
            for (const item of data.minibarConsumptions) {
              await this.addCharge(
                reservation.id,
                `Minibar: ${item.name} (x${item.quantity})`,
  ...
  async startCleaning(id: string, assignedTo: string) {
    return this.updateCleaningRequest(id, {
  ...
  async verifyCleaning(id: string, staffId: string) {
    ...
    await this.cleaningRepo.update(id, {
      status: CleaningStatus.VERIFIED,
      verifiedBy: staff?.name,
      supervisorId: staffId,
    });
    await this.roomRepo.update(request.roomId, {
      status: RoomStatus.AVAILABLE,
    });
  ...
  async assignCleaningRequest(
    ...
    if (isSupervisor) {
      const staff = await this.staffRepo.findOne({ where: { id: staffId } });
      const request = await this.cleaningRepo.findOne({
        where: { id: requestId },
      });
  ...
  async getMaintenanceRequests(companyId: string, status?: string) {
    ...
    return qb
      .orderBy('req.priority', 'DESC')
      .addOrderBy('req.requestedDate', 'ASC')
      .getMany();
  }

  async getMaintenanceStats(companyId: string) {
    const pending = await this.maintenanceRepo.count({
      where: { companyId, status: MaintenanceStatus.PENDING },
    });
    const inProgress = await this.maintenanceRepo.count({
      where: { companyId, status: MaintenanceStatus.IN_PROGRESS },
    });
    const completed = await this.maintenanceRepo.count({
      where: { companyId, status: MaintenanceStatus.COMPLETED },
    });
    const verified = await this.maintenanceRepo.count({
      where: { companyId, status: MaintenanceStatus.VERIFIED },
    });
  ...
  async assignMaintenanceRequest(
    ...
    const staff = await this.maintenanceStaffRepo.findOne({
      where: { id: staffId },
    });
  ...
  async createRestaurantOrder(companyId: string, data: any) {
    ...
    if (data.tableId) {
      await this.tableRepo.update(data.tableId, {
  ...
  async getRestaurantStats(companyId: string) {
    ...
    const inProgress = await this.orderRepo.count({
      where: { companyId, status: OrderStatus.IN_PROGRESS },
    });
    const completed = await this.orderRepo.count({
      where: { companyId, status: OrderStatus.COMPLETED },
    });
    const paid = await this.orderRepo.count({
      where: { companyId, status: OrderStatus.PAID },
    });
    ...
    const totalTips = paidOrders.reduce(
  ...
  async globalSearch(companyId: string, q: string) {
    ...
    const matchedReservations = allReservations.filter(
      (r) =>
  ...
  async createWalkIn(companyId: string, data: any) {
    ...
    if (guest.email) {
      guestEntity = await this.guestRepo.findOne({
  ...
  async getDrawerTransactions(drawerId: string) {
    return this.drawerTransRepo.find({
      where: { drawerId },
      order: { createdAt: 'DESC' },
    });
  }
  ...
  async preAuthorizeCard(
    invoiceId: string,
    data: { amount: number; reference: string; cardLast4: string },
  ) {
    const invoice = await this.invoiceRepo.findOne({
      where: { id: invoiceId },
    });
  ...
  async capturePayment(paymentId: string) {
    ...
    if (!payment.isPreAuthorization)
      throw new Error('No es una pre-autorización');
  ...
  async transferDeposit(
    ...
    const fromRes = await this.reservationRepo.findOne({
      where: { id: data.fromReservationId },
    });
    const toRes = await this.reservationRepo.findOne({
      where: { id: data.toReservationId },
    });

    if (!fromRes || !toRes) throw new Error('Reserva no encontrada');
    if (fromRes.paidAmount < data.amount)
      throw new Error('Depósito insuficiente');
  ...
  async checkSharedAvailability(
    ...
    for (const link of links) {
      const primaryAvail = await this.checkAvailability(
        companyId,
        checkIn,
        checkOut,
        link.primaryRoomTypeId,
      );
      const secondaryAvail = await this.checkAvailability(
        companyId,
        checkIn,
        checkOut,
        link.secondaryRoomTypeId,
      );
