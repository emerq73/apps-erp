...
export class PmsController {
  ...
  createRoomType(@Req() req: any, @Body() body: any) {
    ...
    if (!companyId) {
      throw new Error(
        'No hay empresa seleccionada. Selecciona una empresa primero.',
      );
    }
  ...
  blockRoom(
    @Param('id') id: string,
    @Body() body: { reason: string; blockedFrom: string; blockedTo: string },
  ) {
    return this.pmsService.blockRoom(
      id,
      body.reason,
      new Date(body.blockedFrom),
      new Date(body.blockedTo),
    );
  }
  ...
  calculateRate(
    ...
    return this.pmsService.calculateRate(
      roomTypeId,
      new Date(checkIn),
      new Date(checkOut),
      Number(adults) || 1,
  ...
  createCancellationPolicy(@Req() req: any, @Body() body: any) {
    return this.pmsService.createCancellationPolicy(
  ...
  checkAvailability(
    ...
    return this.pmsService.checkAvailability(
      companyId,
      checkIn,
      checkOut,
      roomTypeId,
  ...
  getCleaningRequests(
    ...
    return this.pmsService.getCleaningRequests(
      this.getCompanyId(req),
      status,
      date,
    );
  ...
  getMaintenanceRequests(@Req() req: any, @Query('status') status?: string) {
    return this.pmsService.getMaintenanceRequests(
      this.getCompanyId(req),
      status,
    );
  }
  ...
  createMaintenanceRequest(@Req() req: any, @Body() body: any) {
    return this.pmsService.createMaintenanceRequest(
      this.getCompanyId(req),
      body,
    );
  }
  ...
  assignMaintenanceRequest(
    @Param('id') id: string,
    @Body() body: { staffId: string; isSupervisor?: boolean },
  ) {
    return this.pmsService.assignMaintenanceRequest(
      id,
      body.staffId,
      body.isSupervisor,
    );
  ...
  assignCleaningRequest(
    ...
    return this.pmsService.assignCleaningRequest(
      id,
      body.staffId,
      body.isSupervisor,
    );
  ...
  openCashDrawer(@Req() req: any, @Body() body: { openingAmount: number }) {
    return this.pmsService.openCashDrawer(
      this.getCompanyId(req),
      body.openingAmount,
      req.user.userId,
    );
  ...
  closeCashDrawer(
    @Req() req: any,
    @Body() body: { actualAmount: number; notes?: string },
  ) {
    const companyId = this.getCompanyId(req);
    if (!companyId) throw new Error('No hay empresa seleccionada');
    const userId = req.user?.userId || '';
    return this.pmsService.closeCashDrawer(
      companyId,
      body.actualAmount,
      body.notes || '',
      userId,
  ...
  addDrawerTransaction(
    ...
    const companyId = this.getCompanyId(req);
    if (!companyId) throw new Error('No hay empresa seleccionada');
    return this.pmsService.addDrawerTransaction(
      companyId,
      body,
      req.user.userId,
    );
  ...
  performAudit(
    @Req() req: any,
    @Body() body: { auditType: string; actualAmount: number; notes?: string },
  ) {
    const companyId = this.getCompanyId(req);
    if (!companyId) throw new Error('No hay empresa seleccionada');
    const userId = req.user?.userId || '';
    return this.pmsService.performAudit(
      companyId,
      body.auditType,
      body.actualAmount,
      body.notes || '',
  ...
  preAuthorizeCard(
    @Param('id') id: string,
    @Body() body: { amount: number; reference: string; cardLast4: string },
  ) {
  ...
  transferDeposit(
    ...
    return this.pmsService.transferDeposit(
      this.getCompanyId(req),
      body,
      req.user.userId,
    );
  ...
  checkSharedAvailability(
    ...
    return this.pmsService.checkSharedAvailability(
      this.getCompanyId(req),
      checkIn,
      checkOut,
    );
