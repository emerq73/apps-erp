...
export class PmsPublicController {
    ...
    async getPublicAvailability(
        ...
        if (!companyId || !checkIn || !checkOut) {
            throw new HttpException('companyId, checkIn y checkOut son requeridos', HttpStatus.BAD_REQUEST);
        }
        ...
        if (coDate <= ciDate) {
            throw new HttpException('La fecha de check-out debe ser posterior al check-in', HttpStatus.BAD_REQUEST);
        }
        ...
        const conflicts = await this.reservationRepo
            ...
            .andWhere('r.status NOT IN (:...excluded)', { excluded: [ReservationStatus.CANCELLED, ReservationStatus.NO_SHOW] })
        ...
        const results = await Promise.all(roomTypes.map(async rt => {
            ...
            try {
                ...
                rateInfo = { nightlyRate: Number(rt.basePrice), totalRate: Number(rt.basePrice) * nights, appliedRate: 'STANDARD' };
            }
    ...
    async getHotelInfo(@Param('companyId') companyId: string) {
        ...
        const totalRooms = roomTypes.reduce((s, rt) => s + (rt.rooms?.length || 0), 0);
        const prices     = roomTypes.map(rt => Number(rt.basePrice)).filter(p => p > 0);
        const minPrice   = prices.length ? Math.min(...prices) : 0;

        return { companyId, totalRoomTypes: roomTypes.length, totalRooms, minPrice, currency: 'DOP' };
    }
    ...
    async createPublicBooking(@Body() body: {
        companyId:      string;
        roomTypeId:     string;
        roomId:         string;
        checkIn:        string;
        ...
        const { companyId, roomId, checkIn, checkOut, adults, children = 0, ratePerNight, specialRequests, guest } = body;

        // Validate required fields
        if (!companyId)         throw new HttpException('companyId es requerido', HttpStatus.BAD_REQUEST);
        if (!roomId)            throw new HttpException('roomId es requerido', HttpStatus.BAD_REQUEST);
        if (!checkIn)           throw new HttpException('checkIn es requerido', HttpStatus.BAD_REQUEST);
        if (!checkOut)          throw new HttpException('checkOut es requerido', HttpStatus.BAD_REQUEST);
        if (!guest?.firstName)  throw new HttpException('Nombre del huésped es requerido', HttpStatus.BAD_REQUEST);
        if (!guest?.lastName)   throw new HttpException('Apellido del huésped es requerido', HttpStatus.BAD_REQUEST);
        if (!guest?.email)      throw new HttpException('Correo del huésped es requerido', HttpStatus.BAD_REQUEST);
