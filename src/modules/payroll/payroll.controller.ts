...
export class PayrollController {
    ...
    private getCompanyId(req: any) {
        return req.user?.companyId || req.session?.companyId || req.headers['x-company-id'];
    }
    ...
    async getEmployees(@Req() req: any, @Query('status') status?: string) {
        return this.payrollService.getEmployees(this.getCompanyId(req), status as any);
    }
    ...
    async terminateEmployee(@Param('id') id: string, @Body('terminationDate') terminationDate: string) {
    ...
    async getRetentionRates(@Req() req: any, @Query('type') type?: string) {
        return this.payrollService.getRetentionRates(this.getCompanyId(req), type as any);
    }

    @Post('retention-rates')
    @ApiOperation({ summary: 'Create retention rate' })
    async createRetentionRate(@Req() req: any, @Body() body: any) {
        return this.payrollService.createRetentionRate(this.getCompanyId(req), body);
    }
    ...
    async createPayrollRun(
        ...
        return this.payrollService.createPayrollRun(this.getCompanyId(req), periodStart, periodEnd);
    }
