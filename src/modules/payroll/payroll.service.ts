...
import { Employee, PayrollRun, PayrollLedger, EmployeeStatus } from '../accounting/entities/payroll.entity';
import { RetentionRate, RetentionType } from '../accounting/entities/retention.entity';
...
export class PayrollService {
    ...
    async getEmployees(companyId: string, status?: EmployeeStatus) {
        ...
        return this.employeeRepo.find({ where, order: { lastName: 'ASC', firstName: 'ASC' } });
    }

    async createEmployee(companyId: string, data: Partial<Employee>) {
        const employee = this.employeeRepo.create({ ...data, company: { id: companyId } });
...
async updateEmployee(id: string, data: Partial<Employee>) {
    ...
    async terminateEmployee(id: string, terminationDate: string) {
        return this.employeeRepo.update(id, { status: EmployeeStatus.TERMINATED, terminationDate });
    }
    ...
    async createRetentionRate(companyId: string, data: Partial<RetentionRate>) {
        const rate = this.retentionRateRepo.create({ ...data, company: { id: companyId } });
    ...
    async calculateRetention(amount: number, type: RetentionType, companyId: string): Promise<number> {
    ...
    // --- PAYROLL RUN ---
    async createPayrollRun(companyId: string, periodStart: string, periodEnd: string) {
        ...
        for (const emp of employees) {
            const ledger = await this.calculatePayrollLedger(emp, savedRun, periodStart, periodEnd);
            ...
            totalEmployerContribution += Number(ledger.employerHealth) + Number(ledger.employerPension) + Number(ledger.employerArl);
        }
    ...
    async calculatePayrollLedger(employee: Employee, payrollRun: PayrollRun, periodStart: string, periodEnd: string) {
        ...
        const totalDeductions = healthEmployee + pensionEmployee + pensionSolidarity;
