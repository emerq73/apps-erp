...
import { Plus, Play, CheckCircle, ShieldCheck, Trash2, Wrench, AlertTriangle, Settings } from 'lucide-react';
...
const MaintenanceManager = () => {
    ...
    const fetchData = async () => {
        ...
        const [r, s] = await Promise.all([
            api.get(`/pms/maintenance-requests${query}`),
            api.get('/pms/maintenance-stats'),
        ]);
        setRequests(r.data);
        setStats(s.data);
    };
    ...
    const fetchRooms = async () => {
        ...
        setRooms(r.data.filter(room => room.status === 'AVAILABLE' || room.status === 'MAINTENANCE'));
    };

    const fetchStaff = async () => {
        const s = await api.get('/pms/maintenance-staff');
        setStaff(s.data);
    };
    ...
    const openStaffForm = (s = null) => {
        setStaffForm(s || { name: '', email: '', phone: '', role: 'TECHNICIAN', isActive: true });
    ...
    const handleSaveStaff = async () => {
        ...
        try {
            if (staffForm.id) {
                await api.put(`/pms/maintenance-staff/${staffForm.id}`, staffForm);
            } else {
                await api.post('/pms/maintenance-staff', staffForm);
            }
    ...
    const deleteStaff = async (id) => {
        const { isConfirmed } = await Swal.fire({ title: '¿Eliminar técnico?', icon: 'warning', showCancelButton: true });
        if (!isConfirmed) return;
        await api.delete(`/pms/maintenance-staff/${id}`);
        fetchStaff();
    };

    const handleSave = async () => {
        if (!form.roomId || !form.issue) { Swal.fire('Error', 'Selecciona habitación y describe el problema', 'warning'); return; }
        try {
            await api.post('/pms/maintenance-requests', form);
            setShowForm(false);
            fetchData();
            fetchRooms();
            Swal.fire('Guardado', '', 'success');
    ...
    const action = async (id, type, request) => {
        ...
        switch (type) {
            case 'start':
                const staff = await api.get('/pms/maintenance-staff');
                const technicians = staff.data.filter(s => s.role === 'TECHNICIAN' || s.role === 'SUPERVISOR');
                if (technicians.length === 0) {
                    Swal.fire('Info', 'No hay técnicos disponibles', 'info');
                    return;
                }
                const { value: staffId } = await Swal.fire({
                    title: 'Asignar a',
                    input: 'select',
                    inputOptions: Object.fromEntries(technicians.map(s => [s.id, s.name])),
                    inputPlaceholder: 'Seleccionar técnico',
                    showCancelButton: true,
                });
                if (!staffId) return;
                res = await api.patch(`/pms/maintenance-requests/${id}/assign`, { staffId, isSupervisor: false });
                break;
            case 'complete':
                res = await api.patch(`/pms/maintenance-requests/${id}/complete`);
                break;
            case 'verify':
                const staff2 = await api.get('/pms/maintenance-staff');
                const supervisors = staff2.data.filter(s => s.role === 'SUPERVISOR');
                ...
                if (!supId) return;
                res = await api.patch(`/pms/maintenance-requests/${id}/assign`, { staffId: supId, isSupervisor: true });
                break;
        }
        fetchData();
        fetchRooms();
    ...
    const handleDelete = async (id) => {
        ...
        if (!isConfirmed) return;
        try {
            await api.delete(`/pms/maintenance-requests/${id}`);
            fetchData();
            fetchRooms();
    ...
    return (
        <div className="fade-in">
            <div style={s.header}>
                <h2 style={s.title}>Mantenimiento</h2>
            ...
            {showForm && (
                <div style={s.formCard}>
                    <h3 style={s.formTitle}>Nueva Solicitud de Mantenimiento</h3>
                    <div style={s.grid}>
                        <div style={s.ig}><label style={s.lbl}>Habitación *</label>
                            <select style={s.inp} value={form.roomId || ''} onChange={e => setForm({...form, roomId: e.target.value})}>
                                <option value="">Seleccionar...</option>
                                {rooms.map(r => <option key={r.id} value={r.id}>#{r.number} - {r.roomType?.name}</option>)}
                            </select>
                        </div>
                        <div style={s.ig}><label style={s.lbl}>Prioridad</label>
                            <select style={s.inp} value={form.priority || 'MEDIUM'} onChange={e => setForm({...form, priority: e.target.value})}>
                                {Object.entries(PRIORITY_CONFIG).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                            </select>
                        </div>
                        <div style={{ ...s.ig, gridColumn: '1 / -1' }}><label style={s.lbl}>Problema *</label><input style={s.inp} value={form.issue || ''} onChange={e => setForm({...form, issue: e.target.value})} placeholder="Ej: Aire acondicionado no funciona"/></div>
                        <div style={{ ...s.ig, gridColumn: '1 / -1' }}><label style={s.lbl}>Descripción</label><textarea style={s.inp} rows={3} value={form.description || ''} onChange={e => setForm({...form, description: e.target.value})} /></div>
                    </div>
            ...
            {showStaffForm && (
                <div style={s.formCard}>
                    <h3 style={s.formTitle}>{staffForm.id ? 'Editar' : 'Nuevo'} Técnico</h3>
                    <div style={s.grid}>
                        ...
                        <div style={s.ig}><label style={s.lbl}>Rol</label>
                            <select style={s.inp} value={staffForm.role || 'TECHNICIAN'} onChange={(e) => setStaffForm({...staffForm, role: e.target.value})}>
            ...
            {!showForm && !showStaffForm && (
                <div style={{ ...s.formCard, marginBottom: '20px', background: '#f8fafc' }}>
                    <h3 style={{ ...s.formTitle, marginBottom: '12px' }}>Técnicos de Mantenimiento</h3>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                        {staff.length === 0 && <div style={{ color: '#94a3b8', padding: '20px 0' }}>No hay técnicos. Agrega miembros del equipo.</div>}
                        {staff.map(s => (
                            <div key={s.id} style={{ ...s.staffChip, border: s.role === 'SUPERVISOR' ? '2px solid #8b5cf6' : '2px solid #f59e0b' }}>
                                <span style={{ fontWeight: '600' }}>{s.name}</span>
                                <span style={{ fontSize: '11px', color: s.role === 'SUPERVISOR' ? '#8b5cf6' : '#f59e0b' }}>
                                    {s.role === 'SUPERVISOR' ? 'SUP' : 'TEC'}
                                </span>
            ...
            <div style={s.list}>
                ...
                {requests.map(req => (
                    <div key={req.id} style={{ ...s.item, borderLeftColor: PRIORITY_CONFIG[req.priority]?.color || '#64748b' }}>
                        <div style={s.itemHeader}>
                            ...
                            <span style={{ ...s.badge, background: '#f3f4f6', color: PRIORITY_CONFIG[req.priority]?.color }}>
                                {PRIORITY_CONFIG[req.priority]?.label}
                            </span>
                            {req.priority === 'URGENT' && <AlertTriangle size={14} color="#dc2626" />}
                        </div>
                        <div style={s.itemDate}>
                            {new Date(req.requestedDate).toLocaleDateString('es-CO')}
                        </div>
                        {req.issue && <div style={{ fontWeight: '600', marginBottom: '4px' }}>{req.issue}</div>}
                        {req.description && <div style={s.itemNotes}>{req.description}</div>}
                        {req.assignedTo && <div style={s.itemAssign}>Asignado a: {req.assignedTo}</div>}
                        <div style={s.itemActions}>
                            {req.status === 'PENDING' && <button style={s.btnAction} onClick={() => action(req.id, 'start')}><Wrench size={14} /> Asignar</button>}
